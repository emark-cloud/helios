...
- **Kite testnet (2368)** — Phase-6 real-price cutover live (broadcast 2026-05-09); full set in `contracts/deployments/kite-testnet.json`. **WS11 V1→V2 cutover live 2026-05-11**: fresh `reputationAnchorV2Bis` `0x2b6c5f3648Ae2aA27c80CB871590D1Ef1346938D` + `strategyRegistryV3` `0xe6c2cfCa8fd59f4b6fCF0b5F83A515aBB7498D35` + `allocatorRegistryV2` `0xb673e6F8f11fb416B47f5d9C0a36400bF9485A06` (registries immutably bound to v2-bis at construction); AllocatorVault + 9 Phase-6 vaults rebound to SR-v3 via `setRegistry`/`setStrategyRegistry` setters added in `allocatorVaultImplWS11` / `strategyVaultImplWS11`; new `heliosOApp` `0x9845c0C697964464dCAF2602b4e516CaEA98E51E` (v2-bis-bound, peers re-wired Base + Arb); subgraph `helios/v0.9.0` (CXR-cost Tier 2 cutover 2026-05-14; v0.8.0 deleted from Goldsky after being superseded so the new entity slot fits under the 3-subgraph free-tier limit. v0.9.0 adds the `CrossChainAllocation` entity + `RemoteAllocationSent` handler so per-strategy cross-chain submits — single-call and batched — are queryable by `txHash` group; consumers compute batch size as the count of siblings sharing one tx hash. Pre-Tier-2 history — v0.7.0 srcChainId crash → v0.7.1 fix in commit `0375944` → v0.7.2 `handleStrategySlashed` in commit `88fc1e5` against phantom EOAs surveyed in `project_phantom_strategy_cleanup` → v0.8.0 CXR-4 chainId init for §12.1 venue routing — retained in commit log only). The legacy `strategyRegistry` (V1), `strategyRegistryV2` (WS9), `reputationAnchor` (V1), and `heliosOApp` predecessor (`0x7Bad5250…` null-anchor) are parked but no longer authoritative — engine + frontend + VPS all point at the WS11 surface. Pinned references:
  ...
  - `strategyRegistry` `0x3a0f5b9436eca0c8c0eced659dcc41e86e65e33d` (V1, Phase-1 deploy; predates `paramsHashOf`/`commitInitialParamsHash` from PR #70 / commit `4674f61`. **Parked/legacy — NOT the active-flag source of truth.** `AllocatorVault.strategyRegistry` is a *mutable* storage var (`AllocatorVault.sol:60`, owner-only `setStrategyRegistry` `:246`), rebound to SR-v3 at WS11; the authoritative active-flag surface for engine + frontend + AllocatorVault is **SR-v3** `0xe6c2cfCa…`. The subgraph still indexes V1 as a datasource, so deactivate/slash events on V1 also propagate to `/strategies` for entries that only ever existed on V1)
  ...
  - Strategy vault impl (UUPS) `0x5d3Ef33068C7ECb5134875B5F4DDEd4197709883` (**defund-unwind rollout 2026-05-17**, commit `6f69747`; supersedes NAV-counter-saturation `0xe13BfCD4…`. Adds privileged proof-less `unwindToBase()` (onlyAllocatorVault, nonReentrant) — swaps every non-base `_manifest.assetUniverse` token's full balance → `baseAsset` via `allowedRouter`, aggregate slippage bounded against the reportNAV-attested mark (`_totalNAV − cash`, `MAX_UNWIND_SLIPPAGE_BPS = 500`, floor enforced only when ≥1 asset swept so a base-only vault that reported a gain is a no-op). `AllocatorVault._unwindAndCredit` calls it FIRST so `defundStrategy`/`finalizeDefund` of an in-position vault no longer revert `ERC20InsufficientBalance` (`0xe450d38c`). A runtime-proven flatten was infeasible — circuit Constraint 6 (`is_exit = is_signal_flip + is_stop_loss`) makes an unconditional flatten EXIT unprovable; a defund is an admin capital-recovery action, not a strategy trade. Function-only, zero storage delta. Same anchors (`priceAnchor 0x566e1F1B…` / `yieldAnchor 0x345cd375…`). All 9 Phase-6 proxies `upgradeToAndCall(impl,"")`'d via `forge script UpgradeKiteDefundUnwind` from owner=deployer EOA `0xECf5e30F…` 2026-05-17 — **forge now installed user-local on the VPS** (`~/.foundry/bin`, no sudo); dry-run proved ownership pre-broadcast; ERC1967 slot read-back verified per proxy. Live-verified: operator `defundStrategy(0x1cFCD4e1…, 0x1717640c…)` tx `0xd22fbafc…` succeeded (had been reverting `0xe450d38c` for hours) — `mWBTC`/`mSOL` → 0, ~922.97 mUSDC stuck principal recovered to the user. 458 forge tests green. Base/Arb deferred (CXR cross-chain defund doesn't route through `_unwindAndCredit`; impl is chain-agnostic so a later redeploy is pure).)
  - Strategy vault impl (UUPS) predecessor `0xe13BfCD4DA759ef986DA7564cd730FBe9f5C1B3C` (**NAV-counter-saturation upgrade 2026-05-16**; supersedes CXR-0b `0xc3C7A30C…`. Only delta vs CXR-0b is `StrategyVault._checkNavDivergence` saturating the `uint8` breach counter at `type(uint8).max` instead of `+1`-overflowing at 255 — the prior unbounded increment Panic-0x11'd and, because reportNAV reverts, permanently bricked NAV reporting (the live yr `0x2aff8735` incident: counter stuck 255, lastNAVTimestamp 38h stale). Same anchors as CXR-0b (`priceAnchor 0x566e1F1B…` / `yieldAnchor 0x345cd375…`) so zero behavior change otherwise; zero storage delta (function-body-only). All 9 active Phase-6 proxies (mom/mr/yr ×{base,V2,V3}) `upgradeToAndCall(impl,"")`'d via web3 from the owner=deployer EOA `0xECf5e30F…` 2026-05-16 — forge missing on VPS, deployed off the local `forge build` artifact; ERC1967 slot read-back verified per proxy. Legacy/phantom proxies intentionally not upgraded. Commit `c06e79e`/main `20d37b0`.)
  - Strategy vault impl (UUPS) predecessor `0xc3C7A30C86186Ae42aad6659bF37D80f1124feA6` (**CXR-0b impl upgrade 2026-05-13**; carries cross-chain hooks `onCrossChainAllocate`/`defundCrossChain` + setter additions `setOperator`/`setNavOracle`/`setRegistry` from commit `27f57bd`. All 9 Phase-6 proxies `upgradeToAndCall`'d in one `UpgradeKiteVaultsToCXR.s.sol` broadcast. Storage drift checked clean; setter zero-guards confirmed via static-call. Superseded by the NAV-counter-saturation impl above 2026-05-16. Supersedes `0xb73994FE…` 2026-05-12 cross-decimal impl which had the 16-PI directional path + decimals-honesty but lacked CXR hooks and signer-rotation setters.)
  - AllocatorVault impl (UUPS) `0x02d4BEC382Cf640BB7CB67c9f49874e581fC43fe` (**defund-unwind rollout 2026-05-17**, commit `6f69747`; supersedes CXR-0b `0x777D56F6…`. Sole delta: a one-line `_unwindAndCredit` hook calling `IStrategyVault(strategy).unwindToBase()` before the `distributeRealized`/`withdrawToAllocator` base-asset sweep. No `defundStrategy`/`finalizeDefund` ABI change. Function-only, zero storage delta. Proxy `0xf3e4452f…` `upgradeToAndCall(impl,"")`'d in the same `UpgradeKiteDefundUnwind` broadcast; ERC1967 read-back verified.)
  - AllocatorVault impl (UUPS) predecessor `0x777D56F67c98519Cf5D3101156e7B7B6693F8e78` (**CXR-0b impl upgrade 2026-05-13**; carries `allocateToRemoteStrategy`/`settleRemoteDefund` cross-chain hooks + `setOperator` setter. Supersedes WS11.3 setter-enabled impl `0x9538e97f463b8fd3862fd577be8101f6519e3b99`. Storage layout append-only — gap shrank 44→41 for `bridgeReceiver`/`oftAdapter`/`_userRemoteDeployed` slots; existing operator/userVault/registry/oracleAnchor preserved.)
  - `reputationAnchor` (V1, registry-bound) `0x51c07adf596b1e72697a9b8232d061ed006943dc`
  - `reputationAnchorV2` (legacy sidecar; superseded by `reputationAnchorV2Bis` at WS11) `0x735680a32a0e5d9d23d7e8e8302f434e7f30428e`
  ...
  - Strategy vaults — **Phase-6 capacity-fix redeploy 2026-05-10** (`DeployPhase6MultiAssetVaults.s.sol`); nine new ERC1967 proxies on impl `0x934f7639e5Cb320e4394736f5663b53E9C6b5c7b`. Universe per class: mom/mr `[mUSDC, mWBTC, mWETH, mSOL]`; yr `[mUSDC]` (Helios.md §12.1 carve-out — yield venues live on Arbitrum). Distinct paramsHash per (class, variant) seeded as `keccak256("helios.<class>.phase6.multiasset.<variant>")`. All active in StrategyRegistry; the prior phase-6 nine are flipped `active=false` (`DeactivateLegacyVaults.s.sol`) — supersedes the 2026-05-09 cohort which used 6-dec `MAX_CAPACITY = 1_000_000e6` against an 18-dec mUSDC, capping each vault at ~1e-6 mUSDC and reverting every Sentinel `allocateToStrategy` with `CapacityExceeded()` (`0x9ff41fe0`). The 2026-05-10 cohort uses `MAX_CAPACITY = 1_000_000e18` and stake `5_000e18`.
    ...
    Allocators auto-resolve through Goldsky's `where: { active: true }` filter (`AllocatorGoldsky.fetch_directory`) — no env-var change to switch the strategy set, the registry is canonical. Subgraph deployed at `helios/v0.9.0` (`https://api.goldsky.com/api/public/project_cmodpmbv1pkd70127d9g741ek/subgraphs/helios/v0.9.0/gn`); Base + Arb venue subgraphs at `helios-base/v0.9.0` + `helios-arbitrum/v0.9.0` under the same project (redeployed to v0.9.0 2026-05-17 — the prior v0.8.0 venue deployments 404'd on Goldsky, breaking the allocator's cross-chain candidate fan-out; `RemoteAllocationSent` fires from the Kite-side AllocatorVault only). The mainnet stretch (if exercised) does a fresh deploy from a clean slate.
  - **YR class on-chain seeding (2026-05-14)** — both yr.kite + yr.arb were registered active but had `paramsHashOf == 0x0` and `marketAllowlistRoot(yr_class) == 0x0`, so `executeYieldRotationWithProof` reverted `ParamsHashNotCommitted` and the auto-commit lifespan hook reverted `NotOperator` (operator EOA was rotated #92 but SR-v3.operator binding still points at deployer). Closed both gaps from the deployer EOA via cast:
    - `paramsHash = 0x1eda06ac97389468feb2a7029a43798a0d0b98222fe4c81ad7d9319b94672d75` (Poseidon([80, 30]) — yr's `signal_threshold_bps=80 + bridging_cost_bps=30` defaults). Committed for yr.kite on SR-v3 (tx `0x1ec193b3…`) and yr.arb on Arb SR (tx `0x0cdbb5a4…`).
    - `marketAllowlistRoot(yr_class=0x2e882135…) = 0x0ee44ff6038010b81e2e310efd8abfeb658b8f8f5e415bc90ac4426683f9c958` (Poseidon depth-4 merkle over markets `[1, 2]` = `aave-v3:USDC, aave-v3:USDT`, padded with `allow_pad()`). Set on Kite SR-v3 (tx `0xf6a7b3af…`) and Arb SR (tx `0x41afd59c…`).
  - **Cross-chain CAPITAL flow is OFF in v1 (decision, 2026-05-17).** Live `OFT.send` capital bridging is disabled by the master kill-switch `cross_chain_capital_enabled` (`LoopConfig`, default `False`) ← `SENTINEL_CROSS_CHAIN_CAPITAL_ENABLED` (default `false`, set `false` on the VPS). The ~1.0–1.2 KITE LZ V2 fixed fee per submit makes per-rebalance bridging impractical on testnet (~35 KITE burned before this cut). With it off, every remote op deterministically becomes a zero-cost `CROSS_CHAIN_ALLOCATION_DEFERRED` event (dashboard still shows cross-chain *intent*; Base/Arb strategies still listed in `/strategies`). **Cross-chain *reputation* propagation is unaffected and stays live — a separate KITE-free path:** rep messages only originate on Base/Arb (`StrategyVault._forwardAttestationIfRemote` is a hard no-op on Kite), so the LZ fee is paid in free Base/Arb Sepolia testnet ETH (~9.9e-5 ETH/msg, measured 2026-05-17; batched + low-cadence), never the scarce KITE the capital `OFT.send` burned. A practical cross-chain capital design is a documented v2 item — see `docs/cross-chain-cost-roadmap.md §"v1 status" / §"v2"`. The CXR-0c gas-budget note below applies *only* if the flag is ever re-enabled for v2 work.
  - **CXR-0c sentinel allocator gas budget** *(dormant while cross-chain capital is OFF — see above)* — `sentinel.operator_pk` EOA `0x0A7d03433CD89827Feb935A80C37c46D58f7dF92` (rotated #93) must hold ≥1.2 KITE per cross-chain submit (LZ V2 testnet executor fee — mostly fixed-cost, not gas-driven; `OFT.quoteSend` returns 1.144 KITE for Arb / 0.995 KITE for Base at 200k/200k lzReceive/lzCompose options). Without sufficient KITE the live submit fails RPC `insufficient funds for transfer` and the loop falls back to a `cross_chain.deferred` event. Top up from the deployer (or testnet faucet at `https://faucet.gokite.ai`) before any §12.1 demo broadcast.
- **Kite mainnet**: *(Stretch — only if time permits. Not in v1 scope.)*
- **Base Sepolia (84532)** — Phase-5 WS10 live-network verification (broadcast 2026-05-11; full set in `contracts/deployments/base-sepolia.json`). Pinned references:
  ...
  - **CXR-3 (2026-05-13)** — Base spot vaults live for §12.1 deep-liquidity routing:
    - `strategyVaultImpl` `0x735680A32A0e5d9d23D7e8e8302F434e7F30428E` (CXR-aware impl with `bridgeReceiver`/`oftAdapter`/`totalCrossChainAllocated` slots — supersedes the Phase-5 `0x78b3515f…` pre-CXR impl. Superseded by the NAV-counter-saturation impl below 2026-05-16.)
    - `strategyVaultImplNavCounterSaturation` `0xe2Cec094113dB7662A5C237a7593e1ee826Bfa8a` (**Base NAV-counter-saturation upgrade 2026-05-16**; CXR + `_checkNavDivergence` uint8-saturation fix (commit `c06e79e`), same Base anchors `priceAnchor 0x6E3C2304…` / `yieldAnchor 0x37714f59…`, zero storage/behavior delta otherwise. `phase6VaultMomentumBase 0x9e14299e…` + `phase6VaultMeanReversionBase 0x8b375617…` `upgradeToAndCall`'d via web3 from owner=deployer `0xECf5e30F…`; ERC1967 slot read-back verified (Base public RPC lags read-after-write — verify slot on a re-read, not immediately post-tx). Mirrors Kite `0xe13BfCD4…` / Arb `0xa57c8f8d…`.)
  ...
  - **CXR Tier-2 batch fix (2026-05-17)** — `heliosBridgeReceiver` `0x43940b36507bF5c135671b9398b2c61557bA55c1` (pro-rata `_allocateBatch`, commit `79c603f`). Supersedes the pre-Tier-2 `0xC8425C93…` (reverted `UnknownAction(2)` on batched composes — deployed at `638a91e`, before `ACTION_ALLOCATE_BATCH`) and the interim batch-aware-but-cross-decimal-buggy `0x77E75a76…` (reverted `BatchAmountMismatch`: the batch payload's `amounts[]` are Kite 18-dec but the OFT delivers `totalAmountLD` in Base 6-dec, so the old exact-equality check was unsatisfiable). The new BR distributes the real OFT-delivered total pro-rata by weight ratios (decimal-agnostic; last entry takes the remainder so the split is exact). `AllocatorVault.destinationReceiver[40245]` re-pointed here on Kite. Verified live: a Tier-2 batch split evenly to mom.base + mr.base (23.57 mUSDC each), BR drained to 0, zero `LzComposeAlert`. Stuck mUSDC in the two predecessors is unrecoverable (their bytecode has no generic owner sweep and `recoverable[user]` was never set — lzCompose reverted at action-dispatch before the per-entry catch) but is testnet mock mUSDC the deployer mints permissionlessly → zero economic loss.
- **Arbitrum Sepolia (421614)** — Phase-5 WS10 live-network verification (broadcast 2026-05-11; full set in `contracts/deployments/arbitrum-sepolia.json`). Pinned references:
  ...
  - **CXR-1 + CXR-2 (2026-05-13)** — first remote execution surface for §12.1 venue routing:
    ...
    - `phase6VaultYieldRotationArb` `0x516f23B9d2b6918D005d00Ccea3074cED1f8D005` (ERC1967 proxy; UUPS-upgraded 2026-05-14 to fresh CXR-aware impl `0x4e6928a2Ac4c9E9dE99d4F33Fa18d31EA0bFA45c` carrying `setOperator`/`setNavOracle`/`setRegistry` setters; supersedes WS11 setter-only impl `0x9C3Eb82e3B17D64aC3957741857962b51A0200E2` so yr.arb's operator + navOracle could be rotated to dedicated EOA `0x86918Bf3cC030688F0d88f9c967646537D506041` off the deployer EOA — task #96); baseAsset = Arb-mUSDC `0xe8CF8A57…` 6-dec; `allowedRouter` = MockYieldVault `0xc065af9b…` Aave-V3-shaped lending venue; `allocatorVault` = deployer EOA placeholder for PI_ALLOCATOR binding until CXR-0b BridgeReceiver lands; stake 5_000 mUSDC, maxCapacity 1_000_000 mUSDC; paramsHash `keccak256("helios.yield_rot_v1.phase6.multiasset.arb")`; class YR; registered on Arb SR; `strategyCount = 1`. Real Aave V3 swap is a one-line `allowedRouter` flip once an Aave Arb-Sepolia faucet becomes accessible — admin-gated FiatToken USDC blocked v1 cutover.
    - `strategyVaultImplCXR` `0x4e6928a2Ac4c9E9dE99d4F33Fa18d31EA0bFA45c` (CXR-aware StrategyVault impl on Arb; 2026-05-14 redeploy carrying setOperator + setNavOracle. Superseded by the NAV-counter-saturation impl below 2026-05-16.)
    - `strategyVaultImplNavCounterSaturation` `0xa57c8f8d96819380A018d24C6718262cc703bBDf` (**Arb NAV-counter-saturation upgrade 2026-05-16**; CXR + `_checkNavDivergence` uint8-saturation fix (commit `c06e79e`), same Arb anchors `priceAnchor 0x6E3C2304…` / `yieldAnchor 0x37714f59…` (identical to Base — same deployer-nonce mock anchors), zero storage/behavior delta otherwise. `phase6VaultYieldRotationArb 0x516f23B9…` `upgradeToAndCall`'d via web3 from owner=deployer `0xECf5e30F…`; ERC1967 slot read-back verified. Mirrors the Kite impl `0xe13BfCD4…`.)
  ...
  - **CXR Tier-2 batch fix (2026-05-17)** — `heliosBridgeReceiver` `0x1540821fcfbac3fA71b3E5e1a479Ea9795A447da` (pro-rata `_allocateBatch`, commit `79c603f`; identical fix to Base). Supersedes pre-Tier-2 `0x49FDD3D9…` + interim `0xe2Cec094…`. `AllocatorVault.destinationReceiver[40231]` re-pointed here on Kite. The Arb single-send path (yr.arb, `ACTION_ALLOCATE`) worked pre-fix because `_allocateOne` uses the OFT-delivered `amountLD` directly and ignores the payload amount; the new BR preserves that and adds correct batched-split support.
...
Phase 6 — Polish + submission. Phases 0–5 complete; `v0.5.0` tagged
Phase 5 cross-chain on 2026-05-08, `v0.6.0-realprice` tagged the
real-price cutover on 2026-05-09, `v0.6.0` tagged Phase 6 acceptance
on 2026-05-14. WS1–WS7 of Phase 5 already on `main` via stacked PRs
#82–#88; WS8 ships `scripts/preflight-phase5.sh`,
