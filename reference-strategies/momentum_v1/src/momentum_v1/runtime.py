...
class MomentumRuntime:
    ...
    def tick_nav(self, total_nav_usd: float, *, timestamp: int | None = None) -> ExecutionRecord:
        ...
        StrategyVault validates the signature against `keccak256(abi.encode(
        vault, totalNAV, timestamp))` recovering to `navOracle`. Phase 1
        signs over the same digest with `nav_oracle_pk`."""
        ...
        # abi.encode(address, uint256, uint256) — left-padded 32-byte words.
        vault_word = (
        ...
        digest = keccak(body)
        # StrategyVault uses raw ECDSA recovery (no EIP-191 prefix), so
        # we use `signHash`-shaped output: r || s || v.
