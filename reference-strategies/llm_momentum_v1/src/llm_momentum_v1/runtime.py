...
from llm_momentum_v1.executor import ExecutionRecord, TradeExecutor
from llm_momentum_v1.oracle_client import OracleClient, OracleEmptyError, SnapshotBundle
from llm_momentum_v1.prover_client import ProofResult, ProverClient, ProverDegraded
from llm_momentum_v1.strategy import LLMMomentumStrategy
from llm_momentum_v1.witness import PRICE_OBSERVATIONS, build_momentum_witness
...
class LLMMomentumRuntime:
    ...
    def start(self) -> None:
        if self._bar_task is None:
            self._stop.clear()
            self._bar_task = asyncio.create_task(self._bar_loop(), name="llm_momentum.bars")
            self._nav_task = asyncio.create_task(self._nav_loop(), name="llm_momentum.nav")
    ...
    async def tick_bar(self) -> list[ExecutionRecord]:
        ...
        for asset in self._strategy.asset_universe:
            ...
            try:
                ...
                _log.warning("llm_momentum.oracle.error", asset=asset, err=str(exc))
    ...
    async def _handle_signal(
        ...
        try:
            ...
            _log.warning("llm_momentum.witness.invalid", asset=asset, err=str(exc))
            self.stats.last_error = str(exc)
            return None
        ...
        try:
            ...
            _log.warning("llm_momentum.prover.degraded", asset=asset, err=str(exc))
    ...
    def _seed_nav_from_chain(self) -> None:
        ...
        try:
            ...
            _log.warning("llm_momentum.nav.seed_failed", err=str(exc))
