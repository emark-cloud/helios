...
def tick(market_id: int, apy_bps: int, ts: int = 1_000_000_000) -> YieldTick:
    return YieldTick(market_id=market_id, apy_bps_e6=apy_bps * 1_000_000, timestamp_ms=ts)
...
def test_ignores_non_allowlisted_markets() -> None:
    ...
    intent = s.on_yield_tick({1: tick(1, 420), 2: tick(2, 550), 3: tick(3, 800)})
    assert intent is not None
...
def test_initial_rotation_picks_worst_as_source() -> None:
    ...
    s.set_capital(10_000)
    intent = s.on_yield_tick({1: tick(1, 420), 2: tick(2, 550), 3: tick(3, 380)})
    assert intent is not None
