...
class MeanReversionRuntime:
    ...
    def tick_nav(self, total_nav_usd: float, *, timestamp: int | None = None) -> ExecutionRecord:
        ...
        StrategyVault validates the signature against
        `keccak256(abi.encode(vault, totalNAV, timestamp))` recovering to
        `navOracle`. Phase 2 reference impl mirrors momentum's signing
        path (no chain-id binding; matches the existing test fixtures).
        """
