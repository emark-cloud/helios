...
///         (`verifyProof(uint[2], uint[2][2], uint[2], uint[14])`) for
///         mean_reversion_v1 onto the dynamic-array `IGroth16Verifier` shape
///         that `TradeAttestationVerifier` calls. Mean-rev shares momentum's
///         14-PI layout (see `StrategyVault.PI_*` constants).
