"""Pydantic models exposed at the REST/WS boundary.
...
`SentinelAllocator.rank_strategies` and `allocate`.

`[PASSPORT-STUB]` — the `signature` field is recorded for forward
compatibility with Kite Passport; Phase 1 does not verify it. See
