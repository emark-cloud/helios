...
- [ ] **Mirror remaining packages to test-PyPI.** CI wiring landed: `.github/workflows/publish-sdk-testpypi.yml` is now a 4-way matrix (`helios-strategy-sdk`, `helios-contracts-abi`, `helios-allocator-sdk`, `helios-trader-cli`) over OIDC trusted publishing, same `testpypi` environment. The three pending packages run with `continue-on-error: true` on the publish step so the workflow stays green until each gets a trusted-publisher entry on test.pypi.org. Flip the matrix `registered:` flag to `true` per package after registration. Box stays unchecked until the next `sdk-v*` tag actually mirrors all four — GitHub Releases remains the live wheel host in the meantime.
