...
- [ ] External contributor could, in principle, publish a new momentum strategy using only the SDK + public docs — PR5.A landed (`d0058af`) — Dockerfile + smoke + fixtures green against the local workspace; PR5.B pivots from test-PyPI to GitHub Releases as the canonical install source (test.pypi.org web-UI gating + the `helios-cli` PyPI name being squatted by an unrelated project, so the CLI publishes as `helios-trader-cli`)
